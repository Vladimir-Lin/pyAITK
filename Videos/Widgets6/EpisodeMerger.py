...
class EpisodeMerger         ( TreeDock                                     ) :
  ...
  def MergeAlbumORGANIZATIONs  ( self , UUID , PUIDs                       ) :
    ...
    return
  ############################################################################
  def MergeAlbumFRAGs          ( self , UUID , PUIDs                       ) :
    ##########################################################################
    DB   = self . ConnectDB    ( UsePure = True                              )
    if                         ( DB in self . EmptySet                     ) :
      return
  ...
  def RunMergeAlbumFRAGs              ( self                               ) :
    ...
    self   . setEnabled               ( False                                )
    VAL    =                          ( UUID , PUIDs ,                       )
    self   . Go                       ( self . MergeAlbumFRAGs , VAL         )
    ##########################################################################
    return
  ############################################################################
  ...
  def Menu                              ( self , pos                       ) :
    ...
    if                                  ( Total > 1                        ) :
      ...
      ########################################################################
      msg  = self . getMenuItem         ( "FragmentMerge"                    )
      ...
      mm   . addActionWithIcon          ( 7011 , icon , msg                  )
      mm   . addSeparator               (                                    )
    ##########################################################################
