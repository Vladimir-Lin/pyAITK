# -*- coding: utf-8 -*-
##############################################################################
## 影片工具
##############################################################################
import os
import sys
import time
import datetime
import logging
import requests
import threading
import gettext
import binascii
import hashlib
import base64
import tempfile
import shutil
import glob
import json
import ffmpeg
import vlc
##############################################################################
from   random                               import seed
from   random                               import randint
from   random                               import randrange
##############################################################################
import urllib
import urllib . parse
from   urllib                               import parse
##############################################################################
import pathlib
from   pathlib                              import Path
##############################################################################
from   io                                   import BytesIO
from   wand   . image                       import Image
from   PIL                                  import Image          as Pillow
##############################################################################
import AITK
##############################################################################
from   AITK   . Database   . Query          import Query
from   AITK   . Database   . Connection     import Connection
from   AITK   . Database   . Pair           import Pair
from   AITK   . Database   . Columns        import Columns
##############################################################################
from   AITK   . Documents  . JSON           import Load           as LoadJson
from   AITK   . Documents  . JSON           import Save           as SaveJson
from   AITK   . Documents  . Name           import Name           as NameItem
from   AITK   . Documents  . Name           import Naming         as Naming
from   AITK   . Documents  . Notes          import Notes          as NoteItem
from   AITK   . Documents  . Variables      import Variables      as VariableItem
from   AITK   . Documents  . ParameterQuery import ParameterQuery as ParameterQuery
##############################################################################
from   AITK   . Calendars  . StarDate       import StarDate       as StarDate
from   AITK   . Calendars  . Periode        import Periode        as Periode
from   AITK   . Essentials . Relation       import Relation       as Relation
##############################################################################
ALBUMVERSION = "2024-12-25-21-11"
##############################################################################
def CurrentAlbumVersion (                                                  ) :
  return ALBUMVERSION
##############################################################################
def SilentRun        ( CMD                                                 ) :
  ############################################################################
  p = os . popen     ( CMD , "r"                                             )
  L = p  . readlines (                                                       )
  p . close          (                                                       )
  ############################################################################
  return L
##############################################################################
def FileStringToWindowsCommand ( FILENAME                                  ) :
  ############################################################################
  F = FILENAME
  F = F . replace              ( "/"  , "\\"                                 )
  F = F . replace              ( "\\" , "\\\\"                               )
  F = F . replace              ( "\"" , "\\\""                               )
  ############################################################################
  return f"\"{F}\""
##############################################################################
def FileStringToWindowsFile ( FILENAME                                     ) :
  ############################################################################
  F = FILENAME
  F = F . replace            ( "\\" , "/"                                    )
  F = F . replace            ( " "  , "\ "                                   )
  F = F . replace            ( "\"" , "\\\""                                 )
  ############################################################################
  return F
##############################################################################
def M3UtoFilms                    ( M3U , PREFIX = True                    ) :
  ############################################################################
  TEXT      = ""
  with open                       ( M3U , "rb" ) as F                        :
    TEXT    = F . read            (                                          )
  ############################################################################
  if                              ( len ( TEXT ) <= 0                      ) :
    return                        [                                          ]
  ############################################################################
  LM        = TEXT . find         ( b'# Playlist created by'                 )
  ############################################################################
  if                              ( LM > 0                                 ) :
    ##########################################################################
    TEXT    = TEXT                [ LM :                                     ]
    LM      = TEXT . find         ( b'\n'                                    )
    ##########################################################################
    if                            ( LM > 0                                 ) :
      TEXT  = TEXT                [ LM + 1 :                                 ]
  ############################################################################
  BODY      = TEXT . decode       ( "utf-8"                                  )
  if                              ( len ( BODY ) <= 0                      ) :
    return                        [                                          ]
  ############################################################################
  MDIR      = os . path . dirname ( os . path . realpath ( M3U )             )
  MDIR      = MDIR . replace      ( "\\" , "/"                               )
  ############################################################################
  LISTs     = BODY . split        ( "\n"                                     )
  FILMs     =                     [                                          ]
  ############################################################################
  for F in LISTs                                                             :
    ##########################################################################
    K       = F
    K       = K . replace         ( "\r" , ""                                )
    K       = K . replace         ( "\n" , ""                                )
    ##########################################################################
    if                            ( len ( K ) <= 0                         ) :
      continue
    ##########################################################################
    if                            ( "#" == K [ 0 : 1 ]                     ) :
      continue
    ##########################################################################
    if                            ( ( "/" in F ) or ( "\\" in F )          ) :
      ########################################################################
      T     = F
      ########################################################################
    else                                                                     :
      ########################################################################
      if                          ( PREFIX                                 ) :
        ######################################################################
        T   = f"{MDIR}/{K}"
        ######################################################################
      else                                                                   :
        ######################################################################
        T   = K
    ##########################################################################
    if                           ( os . path . exists ( T )                ) :
      ########################################################################
      FILMs . append             ( T                                         )
  ############################################################################
  return FILMs
##############################################################################
def GetPosterImages     ( DIR                                              ) :
  ############################################################################
  CWD   = os   . getcwd (                                                    )
  os           . chdir  ( DIR                                                )
  FILEs = glob . glob   ( "*h.jpg"                                           )
  os           . chdir  ( CWD                                                )
  ############################################################################
  return FILEs
##############################################################################
def GetSubfoldersInCurrentDirectory (                                      ) :
  ############################################################################
  Entries     = os . scandir        ( "."                                    )
  FOLDERs     =                     [                                        ]
  ############################################################################
  for Entry in Entries                                                       :
    if Entry  . is_dir              (                                      ) :
      FOLDERs . append              ( Entry . name                           )
  ############################################################################
  return FOLDERs
##############################################################################
def GetFilmsInCurrentDirectory (                                           ) :
  ############################################################################
  VTYPEs =                     [ "mp4"                                     , \
                                 "mkv"                                     , \
                                 "avi"                                     , \
                                 "wmv"                                     , \
                                 "mpg"                                       ]
  FILEs  =                     [                                             ]
  ############################################################################
  for T in VTYPEs                                                            :
    ##########################################################################
    L    = glob . glob         ( f"*.{T}"                                    )
    ##########################################################################
    for F in L                                                               :
      ########################################################################
      if                       ( F not in FILEs                            ) :
        FILEs  . append        ( F                                           )
  ############################################################################
  return sorted                ( FILEs                                       )
##############################################################################
def FullScanFilmsInDirectory                  ( ROOT , DIR                 ) :
  ############################################################################
  VDIR      = f"{ROOT}/{DIR}"
  FILEs     =                                 [                              ]
  CWD       = os . getcwd                     (                              )
  os        . chdir                           ( VDIR                         )
  ############################################################################
  DIRs      = GetSubfoldersInCurrentDirectory (                              )
  WFS       = GetFilmsInCurrentDirectory      (                              )
  ############################################################################
  for W in WFS                                                               :
    ##########################################################################
    FILEs   . append                          ( f"{DIR}/{W}"                 )
  ############################################################################
  for D in sorted                             ( DIRs                       ) :
    ##########################################################################
    WFS     = FullScanFilmsInDirectory        ( ROOT , f"{DIR}/{D}"          )
    ##########################################################################
    for W in WFS                                                             :
      FILEs . append                          ( W                            )
  ############################################################################
  os        . chdir                           ( CWD                          )
  ############################################################################
  return FILEs
##############################################################################
def GetImagesInCurrentDirectory (                                          ) :
  ############################################################################
  VTYPEs =                      [ "png" , "jpg" , "jpeg" , "webp"            ]
  FILEs  =                      [                                            ]
  ############################################################################
  for T in VTYPEs                                                            :
    ##########################################################################
    L    = glob . glob          ( f"*.{T}"                                   )
    ##########################################################################
    for F in L                                                               :
      ########################################################################
      if                        ( F not in FILEs                           ) :
        FILEs . append          ( F                                          )
  ############################################################################
  return FILEs
##############################################################################
def GetAssInCurrentDirectory (                                             ) :
  ############################################################################
  VTYPEs =                   [ "ass"                                         ]
  FILEs  =                   [                                               ]
  ############################################################################
  for T in VTYPEs                                                            :
    ##########################################################################
    L    = glob . glob       ( f"*.{T}"                                      )
    ##########################################################################
    for F in L                                                               :
      ########################################################################
      if                     ( F not in FILEs                              ) :
        FILEs . append       ( F                                             )
  ############################################################################
  return FILEs
##############################################################################
def MergeCoverPosters               ( JPEG                                 ) :
  ############################################################################
  PDIR        = os . path . dirname ( os . path . realpath ( JPEG )          )
  LeftFile    = ""
  RightFile   = ""
  ############################################################################
  if                                ( JPEG . endswith ( "bh.jpg" )         ) :
    ##########################################################################
    LeftFile  = JPEG
    RightFile = JPEG
    RightFile = RightFile . replace ( "bh.jpg" , "h.jpg"                     )
  ############################################################################
  elif                              ( JPEG . endswith ( "h.jpg" )          ) :
    ##########################################################################
    LeftFile  = JPEG
    RightFile = JPEG
    LeftFile  = LeftFile  . replace ( "h.jpg"  , "bh.jpg"                    )
  ############################################################################
  if                                ( len ( LeftFile  ) <= 0               ) :
    return
  ############################################################################
  if                                ( len ( RightFile ) <= 0               ) :
    return
  ############################################################################
  if                                ( not os . path . exists ( LeftFile  ) ) :
    return
  ############################################################################
  if                                ( not os . path . exists ( RightFile ) ) :
    return
  ############################################################################
  CoverFile   = f"{PDIR}/Cover.jpg"
  ############################################################################
  leftImage   = Pillow . open       ( LeftFile                               )
  rightImage  = Pillow . open       ( RightFile                              )
  ############################################################################
  leftSize    = leftImage  . size
  rightSize   = rightImage . size
  ############################################################################
  w           = leftSize [ 0 ] + rightSize [ 0 ]
  h           = leftSize [ 1 ]
  ############################################################################
  if                                ( leftSize [ 1 ] != rightSize [ 1 ]    ) :
    return
  ############################################################################
  newImage    = Pillow . new        ( 'RGB' , ( w , h ) , ( 255,255,255 )    )
  newImage    . paste               ( leftImage  , (            0   , 0    ) )
  newImage    . paste               ( rightImage , ( leftSize [ 0 ] , 0    ) )
  newImage    . save                ( CoverFile  , "JPEG"                    )
  ############################################################################
  return
##############################################################################
def GenerateAlbumFolder  ( DIR                                             ) :
  ############################################################################
  if                     ( not Path ( DIR ) . is_dir ( )                   ) :
    return
  ############################################################################
  ROOTs    =             [ "audios"                                        , \
                           "DP"                                            , \
                           "Features"                                      , \
                           "images"                                        , \
                           "projects"                                      , \
                           "roles"                                         , \
                           "scripts"                                       , \
                           "subtitles"                                     , \
                           "videos"                                          ]
  SUBs     =             [ "CN" , "EN" , "JP" , "TW"                         ]
  ############################################################################
  for F in ROOTs                                                             :
    ##########################################################################
    D      = f"{DIR}/{F}"
    ##########################################################################
    if                   ( not Path ( D ) . is_dir ( )                     ) :
      ########################################################################
      try                                                                    :
        ######################################################################
        os . makedirs    ( D                                                 )
        ######################################################################
      except                                                                 :
        ######################################################################
        pass
  ############################################################################
  ST       = f"{DIR}/subtitles"
  ############################################################################
  if                     ( Path ( ST ) . is_dir ( )                        ) :
    ##########################################################################
    for S in SUBs                                                            :
      ########################################################################
      Z    = f"{ST}/{S}"
      ########################################################################
      if                 ( not Path ( Z ) . is_dir ( )                     ) :
        ######################################################################
        try                                                                  :
          ####################################################################
          os . makedirs  ( Z                                                 )
          ####################################################################
        except                                                               :
          ####################################################################
          pass
  ############################################################################
  return
##############################################################################
def OpenMovieAlbumJson    ( DIR , VUID = "" , TZ = "Asia/Taipei"           ) :
  ############################################################################
  global ALBUMVERSION
  ############################################################################
  if                      ( not Path ( DIR ) . is_dir ( )                  ) :
    return ""
  ############################################################################
  GenerateAlbumFolder     ( DIR                                              )
  ############################################################################
  CWD       = os . getcwd (                                                  )
  os        . chdir       ( DIR                                              )
  SilentRun               ( "move /y *.mp4 videos"                           )
  SilentRun               ( "move /y *.mkv videos"                           )
  SilentRun               ( "move /y *.avi videos"                           )
  SilentRun               ( "move /y *.wmv videos"                           )
  SilentRun               ( "move /y *.mpg videos"                           )
  SilentRun               ( "move /y *.mov videos"                           )
  SilentRun               ( "move /y *.flv videos"                           )
  SilentRun               ( "move /y *.m3u videos"                           )
  SilentRun               ( "move /y *.jpg images"                           )
  SilentRun               ( "move /y *.jpeg images"                          )
  SilentRun               ( "move /y *.png images"                           )
  SilentRun               ( "move /y *.webp images"                          )
  SilentRun               ( "move /y *.nfo scripts"                          )
  SilentRun               ( "move /y *.txt scripts"                          )
  SilentRun               ( "move /y *.torrent scripts"                      )
  os        . chdir       ( CWD                                              )
  ############################################################################
  CHDOC     = f"{DIR}/scripts/Chapters.txt"
  ############################################################################
  if                      ( not Path ( CHDOC ) . is_file (               ) ) :
    ##########################################################################
    BODY    = "Chapters"
    ##########################################################################
    try                                                                      :
      with open           ( CHDOC , 'w' , encoding = "utf-8" ) as File       :
        File . write      ( BODY                                             )
    except                                                                   :
      pass
  ############################################################################
  HDDOC     = f"{DIR}/scripts/head.txt"
  ############################################################################
  if                      ( not Path ( HDDOC ) . is_file (               ) ) :
    ##########################################################################
    BODY    = "\n\n Adult Video"
    ##########################################################################
    try                                                                      :
      with open           ( HDDOC , 'w' , encoding = "utf-8" ) as File       :
        File . write      ( BODY                                             )
    except                                                                   :
      pass
  ############################################################################
  DSDOC     = f"{DIR}/subtitles/Description.txt"
  ############################################################################
  if                      ( not Path ( DSDOC ) . is_file (               ) ) :
    ##########################################################################
    BODY    = "Description"
    ##########################################################################
    try                                                                      :
      with open           ( DSDOC , 'w' , encoding = "utf-8" ) as File       :
        File . write      ( BODY                                             )
    except                                                                   :
      pass
  ############################################################################
  AlbumJson = f"{DIR}/album.json"
  VXID      = f"{VUID}"
  ############################################################################
  if                      ( os . path . exists ( AlbumJson               ) ) :
    ##########################################################################
    if                    ( 19 == len ( VXID )                             ) :
      ########################################################################
      JFV   = LoadJson    ( AlbumJson                                        )
      AB    = ""
      ########################################################################
      if                  ( "Album" in JFV                                 ) :
        ######################################################################
        AB  = JFV         [ "Album"                                          ]
      ########################################################################
      if                  ( AB != VXID                                     ) :
        ######################################################################
        JFV [ "Album" ] = VXID
        ######################################################################
        SaveJson          ( AlbumJson , JFV                                  )
    ##########################################################################
    return AlbumJson
  ############################################################################
  NOW       = StarDate    (                                                  )
  NOW       . Now         (                                                  )
  CFDT      = NOW . toDateTimeString ( TZ , " " , "%Y-%m-%d" , "%H:%M:%S"    )
  ############################################################################
  FOLDER    = os . path . basename ( DIR                                     )
  JFV       =             { "Version"      : ALBUMVERSION                  , \
                            "Album"        : VXID                          , \
                            "Names"        : {                           } , \
                            "Directory"    : DIR                           , \
                            "Folder"       : FOLDER                        , \
                            "Language"     : 1001                          , \
                            "Locality"     : {                           } , \
                            "File"         : AlbumJson                     , \
                            "Vendors"      : [                           ] , \
                            "Images"       : {                           } , \
                            "Videos"       : {                           } , \
                            "Clips"        : {                           } , \
                            "Roles"        : {                           } , \
                            "Links"        : {                           } , \
                            "Documents"    : {                           } , \
                            "Identifiers"  : [                           ] , \
                            "Translations" : {                           } , \
                            "Edited"       : False                         , \
                            "TimeZone"     : TZ                            , \
                            "Creation"     : CFDT                          , \
                            "Modification" : CFDT                          , \
                            "Updated"      : NOW . Stardate                , \
                            "Timestamp"    : NOW . Stardate                  }
  ############################################################################
  SaveJson                ( AlbumJson , JFV                                  )
  ############################################################################
  return AlbumJson
##############################################################################
## 從目錄中產生M3U
##############################################################################
def GenerateFolderM3U                  ( DIR                               ) :
  ############################################################################
  CWD     = os . getcwd                (                                     )
  os      . chdir                      ( DIR                                 )
  ############################################################################
  FILEs   = GetFilmsInCurrentDirectory (                                     )
  ############################################################################
  if                                   ( len ( FILEs ) > 0                 ) :
    ##########################################################################
    FTEXT = "\n" . join                ( FILEs                               )
    ##########################################################################
    with open ( "Album.m3u" , "w" , encoding="utf-8" ) as f                  :
      f   . write                      ( FTEXT                               )
  ############################################################################
  os      . chdir                      ( CWD                                 )
  ############################################################################
  return
##############################################################################
def FullScanFolderM3U                          ( DIR                       ) :
  ############################################################################
  CWD        = os . getcwd                     (                             )
  os         . chdir                           ( DIR                         )
  ############################################################################
  DIRs       = GetSubfoldersInCurrentDirectory (                             )
  FILEs      = GetFilmsInCurrentDirectory      (                             )
  ############################################################################
  for D in sorted                              ( DIRs                      ) :
    ##########################################################################
    WFS      = FullScanFilmsInDirectory        ( DIR , D                     )
    ##########################################################################
    for W in WFS                                                             :
      FILEs  . append                          ( W                           )
  ############################################################################
  if                                           ( len ( FILEs ) > 0         ) :
    ##########################################################################
    WFILEs   =                                 [                             ]
    ##########################################################################
    for F in FILEs                                                           :
      ########################################################################
      W      = F . replace                     ( "/" , "\\"                  )
      WFILEs . append                          ( W                           )
    ##########################################################################
    FTEXT    = "\n" . join                     ( WFILEs                      )
    ##########################################################################
    with open ( "Album.m3u" , "w" , encoding="utf-8" ) as f                  :
      f      . write                           ( FTEXT                       )
  ############################################################################
  os         . chdir                           ( CWD                         )
  ############################################################################
  return
##############################################################################
def FullScanSubfolderM3U                 ( DIR                             ) :
  ############################################################################
  CWD  = os . getcwd                     (                                   )
  os   . chdir                           ( DIR                               )
  ############################################################################
  DIRs = GetSubfoldersInCurrentDirectory (                                   )
  ############################################################################
  for D in sorted                        ( DIRs                            ) :
    ##########################################################################
    FullScanFolderM3U                    ( f"{DIR}/{D}"                      )
  ############################################################################
  os   . chdir                           ( CWD                               )
  ############################################################################
  return
##############################################################################
## 取得目錄功能資訊
##############################################################################
def RetrieveDirectoryInformation ( DIR                                     ) :
  ############################################################################
  DJSON  = f"{DIR}/Directory.json"
  JSOX   =                       {                                           }
  ############################################################################
  if                             ( os . path . exists ( DJSON            ) ) :
    ##########################################################################
    JSOX = LoadJson              ( DJSON                                     )
  ############################################################################
  return JSOX
##############################################################################
## 檢查目錄是否為影片
##############################################################################
def isFilmDirectory ( DIR                                                  ) :
  ############################################################################
  FN   = f"{DIR}/album.json"
  ############################################################################
  if                ( not Path ( FN ) . is_file (                        ) ) :
    return False
  ############################################################################
  JFV  = LoadJson   ( FN                                                     )
  ############################################################################
  if                ( "Album" not in JFV                                   ) :
    return False
  ############################################################################
  FUID = JFV        [ "Album"                                                ]
  ############################################################################
  if                ( 19 != len ( FUID )                                   ) :
    return False
  ############################################################################
  return   True
##############################################################################
## 產生影片集結目錄資訊
##############################################################################
def GenerateCollectionDirectory      ( DIR                                 , \
                                       TZ   = "Asia/Taipei"                , \
                                       LOGF = None                         ) :
  ############################################################################
  JSN = RetrieveDirectoryInformation ( DIR                                   )
  ############################################################################
  if                                 ( "Multimedia" in JSN                 ) :
    return
  ############################################################################
  BNAME = os . path . basename       ( DIR                                   )
  ############################################################################
  if                                 ( "Directory" not in JSN              ) :
    JSN [ "Directory"  ] = BNAME
  ############################################################################
  if                                 ( "Multimedia" not in JSN             ) :
    JSN [ "Multimedia" ] =           { "Directories" : [                   ] }
  ############################################################################
  DJSON = f"{DIR}/Directory.json"
  SaveJson                           ( DJSON , JSN                           )
  ############################################################################
  return
##############################################################################
## 產生製片商目錄資訊
##############################################################################
def GenerateVendorDirectory                   ( DIR                        , \
                                                CUID = ""                  , \
                                                TZ   = "Asia/Taipei"       , \
                                                INFO = {                 } , \
                                                LOGF = None                ) :
  ############################################################################
  CXID      = f"{CUID}"
  JSN       = RetrieveDirectoryInformation    ( DIR                          )
  ############################################################################
  NAMEs     =                                 {                              }
  ############################################################################
  if                                          ( "Names" in INFO            ) :
    NAMEs   = INFO                            [ "Names"                      ]
  ############################################################################
  if                                          ( "Vendor" in JSN            ) :
    ##########################################################################
    if                                        ( 19 == len ( CXID )         ) :
      JSN [ "Vendor" ] [ "Names" ] = NAMEs
    ##########################################################################
  else                                                                       :
    ##########################################################################
    BNAME   = os . path . basename            ( DIR                          )
    ##########################################################################
    if                                        ( "Directory" not in JSN     ) :
      JSN [ "Directory" ] = BNAME
    ##########################################################################
    if                                        ( "Vendor" not in JSN        ) :
    ##########################################################################
      JSN [ "Vendor"    ] =                   { "Name"  : BNAME            , \
                                                "Names" : {                } }
    ##########################################################################
    if                                        ( 19 == len ( CXID )         ) :
      ########################################################################
      JSN [ "Vendor" ] [ "Uuid" ] = CUID
  ############################################################################
  CWD       = os . getcwd                     (                              )
  os        . chdir                           ( DIR                          )
  DIRs      = GetSubfoldersInCurrentDirectory (                              )
  os        . chdir                           ( CWD                          )
  FILMs     =                                 [                              ]
  EE        =                                 [ False , None                 ]
  ############################################################################
  for F in DIRs                                                              :
    ##########################################################################
    FN      = f"{DIR}/{F}"
    ##########################################################################
    if                                        ( isFilmDirectory ( FN )     ) :
      ########################################################################
      FILMs . append                          ( F                            )
      ########################################################################
      if                                      ( LOGF not in EE             ) :
        ######################################################################
        LOGF                                  ( F                            )
  ############################################################################
  JSN [ "Directories" ] = DIRs
  JSN [ "Films"       ] = FILMs
  ############################################################################
  DJSON     = f"{DIR}/Directory.json"
  SaveJson                                    ( DJSON , JSN                  )
  ############################################################################
  return
##############################################################################
## 產生影片系列目錄資訊
##############################################################################
def GenerateVideoSeriesDirectory       ( DIR , TZ = "Asia/Taipei"          ) :
  ############################################################################
  JSN   = RetrieveDirectoryInformation ( DIR                                 )
  ############################################################################
  if                                   ( "Series" in JSN                   ) :
    return
  ############################################################################
  BNAME = os . path . basename         ( DIR                                 )
  ############################################################################
  if                                   ( "Directory" not in JSN            ) :
    JSN [ "Directory" ] = BNAME
  ############################################################################
  if                                   ( "Series" not in JSN               ) :
    JSN [ "Series"    ] =              { "Name"        : BNAME             , \
                                         "Names"       : {               } , \
                                         "Directories" : [               ]   }
  ############################################################################
  DJSON = f"{DIR}/Directory.json"
  SaveJson                             ( DJSON , JSN                         )
  ############################################################################
  return
##############################################################################
##############################################################################
##############################################################################
##############################################################################
##############################################################################
##############################################################################
##############################################################################
##############################################################################
##############################################################################
##############################################################################
##############################################################################
##############################################################################
