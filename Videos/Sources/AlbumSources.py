...
class AlbumSources       ( TreeDock                                        ) :
  ...
  def AssureUrlItem                  ( self , item , uuid , URL            ) :
    ...
    return
  ############################################################################
  def DoDownload                     ( self , item , FUNC                  ) :
    ##########################################################################
    if                               ( "Program" not in self . Settings    ) :
      return
    ##########################################################################
    AITKROOT = self . Settings       [ "Program"                             ]
    AITKROOT = os . path . abspath   ( f"{AITKROOT}/../"                     )
    ##########################################################################
    VUID     = self . Relation . get ( "first"                               )
    WUID     = item . data           ( 0 , Qt . UserRole                     )
    URL      = item . text           ( 0                                     )
    ##########################################################################
    OPTs     =                       ( AITKROOT                            , \
                                       VUID                                , \
  ...
  def StartDownload ( self , item                                          ) :
    ...
    return
  ############################################################################
  def ReportUrlOwners                    ( self , uuid , url               ) :
    ##########################################################################
    DB     = self . ConnectDB            ( UsePure = True                    )
    if                                   ( self . NotOkay ( DB )           ) :
      return
    ...
    ##########################################################################
    self          . Relation . set       ( "second" , uuid                   )
    OWNERs = self . Relation . GetOwners ( DB , RELTAB                       )
    ##########################################################################
    DB     . Close                       (                                   )
    ##########################################################################
    URLz   = json . dumps                ( OWNERs                            )
    self   . emitLog . emit              ( f"{url} : {URLz}"                 )
    ##########################################################################
    return
  ############################################################################
  def ReportOwners     ( self , item                                       ) :
    ##########################################################################
    url  = item . text ( 0                                                   )
    uuid = item . data ( 0 , Qt . UserRole                                   )
    uuid = int         ( uuid                                                )
    ##########################################################################
    self . Go          ( self . ReportUrlOwners , ( uuid , url ,           ) )
    ##########################################################################
    return
  ############################################################################
  def Menu                             ( self , pos                        ) :
