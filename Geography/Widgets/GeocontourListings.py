...
class GeocontourListings           ( TreeDock                              ) :
  ...
  def DecodeFromText      ( self , BODY                                    ) :
    ...
    return KMLs
  ############################################################################
  def DecodeFromKml              ( self , Filename                         ) :
    ##########################################################################
    try                                                                      :
      ########################################################################
      with open                  ( Filename,"r",encoding="utf-8" ) as f      :
        KRM  = kmlparser . parse ( f ) . getroot (                           )
      ########################################################################
      for place in KRM . Document . Placemark                                :
        ######################################################################
        CT = place.Polygon.outerBoundaryIs.LinearRing.coordinates.text.strip()
        return self . DecodeFromText ( CT                                    )
    ##########################################################################
    except                                                                   :
      return                     [                                           ]
    ##########################################################################
  ...
  def LoadFromFilename                     ( self , uuid , Filename        ) :
    ...
    if                                     ( ".kml" in LNAME               ) :
      ########################################################################
      SPOTs     = self . DecodeFromKml     ( Filename                        )
    ##########################################################################
    elif                                   ( ".txt" in LNAME               ) :
      ########################################################################
