...
class TaskListings                 ( TreeDock                              ) :
  ...
  def EventsJoinTask          ( self , UUID , UUIDs                        ) :
    ...
    ##########################################################################
    DB     . LockWrites       ( [ RELTAB                                   ] )
    TASK   . JoinEvents       ( DB , RELTAB , UUIDs                          )
    DB     . UnlockTables     (                                              )
    ##########################################################################
    DB     . Close            (                                              )
    ##########################################################################
  ...
  def UpdateTypeItemValue         ( self   , uuid , item , value           ) :
    ##########################################################################
    TSKTAB = self . Tables        [ "Tasks"                                  ]
    self   . UpdateTableItemValue ( TSKTAB , uuid , item , value             )
    ##########################################################################
    ## 發送訊息給計畫管理後台
    ## SendWssBack
    ##########################################################################
  ...
  def RunGroupsMenu                ( self , at , item                      ) :
    ...
    if                             ( at == 1512                            ) :
      ...
      uuid = self . itemUuid       ( item , 0                                )
      name = item . text           ( 1                                       )
      self . NodeDependency . emit ( name , 16 , str ( uuid ) , 32           )
      ########################################################################
      return True
    ##########################################################################
    if                             ( at == 1521                            ) :
