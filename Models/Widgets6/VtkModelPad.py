...
class VtkModelPad                  ( QStackedWidget , VirtualGui           ) :
  ...
  def spotActor                             ( self , ID                    ) :
    ...
    return actor
  ############################################################################
  def actorVisibility ( self , state                                     ) :
    ##########################################################################
    ID = self . ui . ActorsBox . currentData (                               )
    ID = int                                 ( ID                            )
    ##########################################################################
    if                                       ( ID < 0                      ) :
      return
    ##########################################################################
    actor  = self . spotActor                ( ID                            )
    ##########################################################################
    if                                       ( actor in [ False , None ]   ) :
      return
    ##########################################################################
    visible   = False
    ##########################################################################
    ...
    return
  ############################################################################
  def actorChanged            ( self , AT                                  ) :
    ##########################################################################
    ID     = self . ui . ActorsBox . itemData ( AT                           )
    ID     = int              ( ID                                           )
    actor  = self . spotActor ( ID                                           )
    ##########################################################################
    if                        ( actor in [ False , None ]                  ) :
    ...
    if                        ( visible                                    ) :
      state = Qt . Checked
    ##########################################################################
    self . ui . ActorVisibility . setCheckState ( state                      )
    ##########################################################################
    return
  ############################################################################
