...
##############################################################################
class EthnologueListings           ( TreeDock                              ) :
  ############################################################################
  ...
  def loading                         ( self                               ) :
    ...
    ##########################################################################
    ETNTAB  = self . Tables           [ "Ethnologue"                         ]
    ##########################################################################
    for UUID in UUIDs                                                        :
      ########################################################################
      QQ    = f"""select
                  `id`,`prefer`,`iso639-3`,`alias`,`english`,`native`,`note`
                  from {ETNTAB}
                  where ( `uuid` = {UUID} ) ;"""
      QQ    = " " . join              ( QQ . split ( )                       )
      DB    . Query                   ( QQ                                   )
      ...
      if ( ( RR not in [ False , None ] ) and ( len ( RR ) == 7 ) )          :
        ...
        J [ "Id"      ] = int         ( RR [ 0 ]                             )
        J [ "Prefer"  ] = int         ( RR [ 1 ]                             )
        J [ "ISO"     ] = self . assureString ( RR [ 2 ]                     )
        J [ "Alias"   ] = self . assureString ( RR [ 3 ]                     )
        J [ "English" ] = self . assureString ( RR [ 4 ]                     )
        J [ "Native"  ] = self . assureString ( RR [ 5 ]                     )
        J [ "Note"    ] = self . assureString ( RR [ 6 ]                     )
        J [ "Name"    ] = NAMEs       [ UUID                                 ]
        LISTs           . append      ( J                                    )
    ##########################################################################
    self    . setVacancy              (                                      )
