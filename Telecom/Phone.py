...
class Phone              ( Columns                                         ) :
  ...
  def UpdatePhone      ( self , DB , TABLE ) :
    ...
    return True
  ############################################################################
  def UpdateProperties ( self , DB , TABLE                                 ) :
    ##########################################################################
    UUID    = self . Uuid
    CORRECT = self . Correct
    MOBILE  = self . Mobile
    SHARE   = self . Shareable
    CONFIRM = self . Confirm
    REGION  = self . Region
    ##########################################################################
    QQ      = f"""update {TABLE}
    ...
    return True
  ############################################################################
  def UpdateFull            ( self , DB , PhoneTable , PropertiesTable     ) :
    ##########################################################################
    self . UpdatePhone      (        DB , PhoneTable                         )
    self . UpdateProperties (        DB ,              PropertiesTable       )
    ##########################################################################
    return True
  ############################################################################
  def isCorrect   ( self                                                   ) :
  ...
  def setFull                      ( self , full                           ) :
    ...
    F                = F . replace ( " "  , "-"                              )
    F                = F . replace ( "\t" , "-"                              )
    if                             ( not self . isPhoneFormat ( F )        ) :
      return False
    ##########################################################################
    try                                                                      :
      ppn            = phonenumbers . parse ( F                              )
    except                                                                   :
      return False
    ##########################################################################
    F                = F . replace ( "+" , ""                                )
