...
class PositionListings             ( TreeDock                              ) :
  ...
  def FixPositions           ( self                                        ) :
    ...
    POSTAB = self . Tables    [ "Positions"                                  ]
    ##########################################################################
    QQ     = f"select `id` from {POSTAB} order by `id` asc ;"
    UUIDs  = DB . ObtainUuids ( QQ                                           )
    ##########################################################################
    for UUID in UUIDs                                                        :
      ########################################################################
      QQ   = f"select `latitude` , `longitude` from {POSTAB} where ( `id` = {UUID} ) ;"
      DB   . Query            ( QQ                                           )
      RR   = DB . FetchOne    (                                              )
      ########################################################################
      LATI = RR               [ 0                                            ]
      LONG = RR               [ 1                                            ]
      ########################################################################
      MLAT = self . toLati    ( LATI                                         )
      MLON = self . toLong    ( LONG                                         )
      ########################################################################
      QQ   = f"update {POSTAB} set `latistr` = %s , `longstr` = %s where ( `id` = {UUID} ) ;"
      DB   . QueryValues ( QQ , ( MLAT , MLON , ) )
      print(UUID,LATI,LONG,MLAT,MLON)
    ##########################################################################
    DB     . Close            (                                              )
    ##########################################################################
    self   . Notify           ( 5                                            )
