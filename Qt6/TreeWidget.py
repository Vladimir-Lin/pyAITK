...
class TreeWidget              ( QTreeWidget , VirtualGui                   ) :
  ############################################################################
  emitPendingTopLevelItem = Signal ( QTreeWidgetItem                         )
  pendingRemoveItem       = Signal ( QTreeWidgetItem                         )
  pickSelectionMode       = Signal ( str                                     )
  SubmitStatusMessage     = Signal ( str , int                               )
  SubmitTtsTalk           = Signal ( str , int                               )
  SubmitUpdate            = Signal (                                         )
  emitAdjustWidths        = Signal (                                         )
  emitAutoFit             = Signal (                                         )
  ...
  OnBusy                  = Signal (                                         )
  GoRelax                 = Signal (                                         )
  emitAssignColumn        = Signal ( QTreeWidgetItem , int , str             )
  ...
  def CreateDragMime                 ( self                                , \
    ...
    ##########################################################################
    if                               ( icon  not in self . EmptySet        ) :
      ########################################################################
      s       = QSize                ( 128 , 128                             )
      image   = icon . pixmap ( s ) . toImage (                              )
      ########################################################################
      if                             ( image not in self . EmptySet        ) :
        ######################################################################
  ...
  def itemAtPos              ( self , pos , column , tcolumn               ) :
    ...
    if                       ( self . IsOkay ( atItem )                    ) :
      ########################################################################
      KTID   = atItem . data ( column , Qt . UserRole                        )
