...
class Relation           ( Columns                                         ) :
  ...
  ############################################################################
  def LastReverse          ( self , Table                                  ) :
    WS = self . SecondItem ( "order by `reverse` desc" , "limit 0,1"         )
    return f"select `reverse` from {Table} {WS} ;"
  ############################################################################
  ...
  def Append           ( self , DB , Table                                 ) :
    ...
    return DB . Query  ( QQ                                                  )
  ############################################################################
  def GetLastestPosition ( self , DB , Table                               ) :
    ##########################################################################
    ID   = -1
    ##########################################################################
    QQ   = self . Last   ( Table                                             )
    ...
    return ID
  ############################################################################
  def GetLastestReverse       ( self , DB , Table                          ) :
    ##########################################################################
    ID   = -1
    ##########################################################################
    QQ   = self . LastReverse ( Table                                        )
    DB   . Query              ( QQ                                           )
    rr   = DB . FetchOne      (                                              )
    ##########################################################################
    if                        ( not rr                                     ) :
    ...
    return ID
  ############################################################################
  def UpdateReverse       ( self , DB , Table                              ) :
    ##########################################################################
    R  = self . Reverse
    WS = self . ExactItem ( "" , ""                                          )
    QQ = f"update {Table} set `reverse` = {R} {WS} ;"
    ##########################################################################
    DB . Query            ( QQ                                               )
    ##########################################################################
    return
  ...
  def Join                     ( self , DB , Table                         ) :
    ...
    if ( not Lists ) or ( Lists == None ) or ( len ( Lists ) <= 0 )          :
      ...
      return False
    ##########################################################################
    self . Position = self . GetLastestPosition ( DB , Table                 )
    ##########################################################################
  ...
  def Joins       ( self , DB , Table , Lists                              ) :
    ...
    return
  ############################################################################
  def JoinsFirst             ( self , DB , Table , Lists                   ) :
    ##########################################################################
    for x in Lists                                                           :
      ########################################################################
      self   . set           ( "first" , x                                   )
      self   . Reverse = self . GetLastestReverse ( DB , Table               )
      ########################################################################
      OK     = self . Join   ( DB      , Table                               )
      ########################################################################
      if                     ( OK not in [ False ]                         ) :
        self . UpdateReverse ( DB      , Table                               )
    ##########################################################################
    return
  ############################################################################
  ...
  def Ordering                ( self , DB , Table , UUIDs                  ) :
    ...
    return True
  ############################################################################
  def GetSecondUuidsSqlSyntaxByValue ( self , Table , VALUE                ) :
    ##########################################################################
    TT1 = self . T1
    TT2 = self . T2
    RRR = self . Relation
    ##########################################################################
    QQ  = f"""select `second` from {Table}
              where ( `first` {VALUE} )
                and ( `t1` = {TT1} )
                and ( `t2` = {TT2} )
                and ( `relation` = {RRR} )"""
  ...
  ############################################################################
  def GetSecondUuidsSqlSyntax ( self , Table                               ) :
    ##########################################################################
    FUID = self . First
    ##########################################################################
    return self .GetSecondUuidsSqlSyntaxByValue ( Table , f" = {FUID}"       )
  ############################################################################
  def GetFirstUuidsSqlSyntaxByValue ( self , Table , VALUE                 ) :
    ##########################################################################
    TT1 = self . T1
    TT2 = self . T2
    RRR = self . Relation
    ##########################################################################
    QQ  = f"""select `first` from {Table}
              where ( `second` {VALUE} )
                and ( `t1` = {TT1} )
                and ( `t2` = {TT2} )
                and ( `relation` = {RRR} )"""
    ...
    return " " . join               ( QQ . split (                         ) )
  ############################################################################
  def GetFirstUuidsSqlSyntax ( self , Table                                ) :
    ##########################################################################
    TUID = self . Second
    ##########################################################################
    return self .GetFirstUuidsSqlSyntaxByValue ( Table , f" = {TUID}"        )
##############################################################################
