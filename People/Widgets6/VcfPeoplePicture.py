...
class VcfPeoplePicture           ( VcfPicture                              ) :
  ...
  def PrepareForActions                    ( self                          ) :
    ...
    self . AppendSideActionWithIcon        ( "SquareFacial"                , \
                                             ...
                                             True                          , \
                                             False                           )
    self . AppendSideActionWithIcon        ( "FastSquareFacial"            , \
                                             ":/images/human.png"          , \
                                             self . RunFastSquareFacial    , \
                                             True                          , \
                                             False                           )
    ##########################################################################
    return
  ...
  def FastSquareFacial                      ( self                         ) :
    ...
    if                                      ( len ( FACEs ) > 0            ) :
      ...
      for F in FACEs                                                         :
        ...
        FACE = FaceItem                     (                                )
        FACE . setFull                      ( WW , HH                        )
        ZF   = FACE . ToSquareRestraint     ( F , self . SquareFactor        )
        ######################################################################
        XX   = ZF                           [ "X"                            ]
        YY   = ZF                           [ "Y"                            ]
        ZW   = ZF                           [ "W"                            ]
        ZH   = ZF                           [ "H"                            ]
        ######################################################################
        PIC  = self . PICOP . Crop          ( XX , YY , ZW , ZH              )
        ######################################################################
        LMAS = RECG . DoDetectFaceLandmarks ( PIC                            )
        ######################################################################
        if LMAS                             [ "Found"                      ] :
          ####################################################################
          LM = LMAS                         [ "Landmarks"                    ]
          ...
          GW = RZ                           [ "W"                            ]
          GH = RZ                           [ "H"                            ]
          MH = float                        ( GH * self . BestFactor         )
          SY = float                        ( SY - MH + GH                   )
          FX = float                        ( XX + SX                        )
          FY = float                        ( YY + SY                        )
          RW = RECG . CreateRectangle       ( FX , FY , GW , MH              )
          MT = FACE . ToSquareRestraint     ( RW , 1.05                      )
    ##########################################################################
    self     . Gui . GoRelax . emit         (                                )
    self     . CallGeometryChange           (                                )
    self     . Notify                       ( 5                              )
    ...
    if                                      ( MT not in self . EmptySet    ) :
      ########################################################################
      RK     = self . RectangleToQRect      ( MT                             )
      self   . AddFaceRegion                ( RK                             )
    ##########################################################################
    return
  ############################################################################
  def RunFastSquareFacial ( self                                           ) :
    ##########################################################################
    self . Go             ( self . FastSquareFacial                          )
    ##########################################################################
    return
  ############################################################################
  ...
  def PeopleFaceMenu                  ( self , mm , Menu                   ) :
    ...
    ##########################################################################
    MSG   = self . getMenuItem        ( "FastSquareFacial"                   )
    ICON  = QIcon                     ( ":/images/human.png"                 )
    mm    . addActionFromMenuWithIcon ( LOM , 98438503 , ICON , MSG          )
    ##########################################################################
    return
  ############################################################################
