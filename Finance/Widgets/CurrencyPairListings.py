...
class CurrencyPairListings         ( TreeDock                              ) :
  ...
  def AssureUuidItem          ( self , item , column , uuid , name         ) :
    ...
    if                        ( REV                                        ) :
      ########################################################################
      QQ   = f"""select `source`,`target`,`reverse` from {CRYTAB}
                 where ( `uuid` = {uuid} ) ;"""
      QQ   = " " . join       ( QQ . split ( )                               )
      DB   . Query            ( QQ                                           )
      RR   = DB . FetchOne    (                                              )
      ########################################################################
      if ( ( self . IsOkay ( RR ) ) and ( len ( RR ) == 2 ) )                :
        ######################################################################
        FUID = int            ( R [ 0 ]                                      )
        TUID = int            ( R [ 1 ]                                      )
        RUID = int            ( R [ 2 ]                                      )
      ...
      if                      ( RUID > 0                                   ) :
        ...
        RR = DB . FetchOne    (                                              )
        ######################################################################
        if ( ( self . IsOkay ( RR ) ) and ( len ( RR ) == 1 ) )              :
          ####################################################################
          KUID = int          ( R [ 0 ]                                      )
    ##########################################################################
