...
class HostsWidget                  ( TreeDock                              ) :
  ...
  def PrepareItem                  ( self , JSON                           ) :
    ...
    ##########################################################################
    IT       . setText             ( 1 , str ( DNAME   )                     )
    IT       . setTextAlignment    ( 1 , Qt . AlignRight                     )
    ##########################################################################
    IT       . setText             ( 2 , str ( PAGES   )                     )
  ...
  def loading                         ( self                               ) :
    ...
    for U in UUIDs                                                           :
      ...
      ########################################################################
      QQ    = f"""select
                  `id`,`used`,`name`,`reverse`,`domain`,`pages`
                  from {HSTTAB}
                  where ( `uuid` = {U} ) ;"""
      QQ    = " " . join              ( QQ . split ( )                       )
      DB    . Query                   ( QQ                                   )
      ...
      if ( ( RR not in [ False , None ] ) and ( len ( RR ) == 6 ) )          :
        ...
        if                            ( DUID > 0                           ) :
          ...
          if ( ( RR not in [ False , None ] ) and ( len ( RR ) == 1 ) )      :
            ##################################################################
            J [ "DName" ] = self . assureString ( RR [ 0 ]                   )
      ########################################################################
      IMACT . append                  ( J                                    )
    ##########################################################################
    DB      . Close                   (                                      )
  ...
  def FetchRegularDepotCount   ( self , DB                                 ) :
    ##########################################################################
    HSTTAB = self . Tables     [ "Hosts"                                     ]
    QQ     = f"select count(*) from {HSTTAB} where ( `used` > 0 ) ;"
    DB     . Query             ( QQ                                          )
    ONE    = DB . FetchOne     (                                             )
    ##########################################################################
    if                         ( ONE == None                               ) :
  ...
  def Prepare                    ( self                                    ) :
    ...
    LABELs = self . Translations [ "HostsWidget" ] [ "Labels"                ]
  ...
  def CopyToClipboard        ( self                                        ) :
    ...
    return
  ############################################################################
  def HostBelongings             ( self , DB , Uuid                        ) :
    ##########################################################################
