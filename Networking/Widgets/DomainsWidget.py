...
class DomainsWidget                ( TreeDock                              ) :
  ...
  def loading                         ( self                               ) :
    ...
    for U in UUIDs                                                           :
      ...
      DB    . Query                   ( QQ                                   )
      RR    = DB . FetchOne           (                                      )
      if ( ( RR not in [ False , None ] ) and ( len ( RR ) == 8 ) )          :
        ######################################################################
        J [ "Id"      ] = int         ( RR [ 0 ]                             )
        J [ "Used"    ] = int         ( RR [ 1 ]                             )
        J [ "Name"    ] = self . assureString ( RR [ 2 ]                     )
        J [ "Reverse" ] = self . assureString ( RR [ 3 ]                     )
        J [ "TLD"     ] = int         ( RR [ 4 ]                             )
        J [ "SLD"     ] = int         ( RR [ 5 ]                             )
        J [ "Hosts"   ] = int         ( RR [ 6 ]                             )
        J [ "Pages"   ] = int         ( RR [ 7 ]                             )
        ...
        if                            ( TXID > 0                           ) :
          ####################################################################
          TXID          = 8300000000001000000 + TXID
          QQ            = f"""select `name` from {TLDTAB}
                              where ( `uuid` = {TXID} ) ;"""
          QQ            = " " . join  ( QQ . split ( )                       )
          DB            . Query       ( QQ                                   )
          RR            = DB . FetchOne (                                    )
          ...
          if ( ( RR not in [ False , None ] ) and ( len ( RR ) == 1 ) )      :
            J [ "TName" ] = self . assureString ( RR [ 0 ]                   )
        ######################################################################
        SXID            = J           [ "SLD"                                ]
        if                            ( SXID > 0                           ) :
          ####################################################################
          SXID          = 8300001000000000000 + SXID
          ####################################################################
          QQ            = f"""select `name` from {SLDTAB}
                              where ( `uuid` = {SXID} ) ;"""
          QQ            = " " . join  ( QQ . split ( )                       )
          DB            . Query       ( QQ                                   )
          RR            = DB . FetchOne (                                    )
          ####################################################################
          if ( ( RR not in [ False , None ] ) and ( len ( RR ) == 1 ) )      :
            J [ "SName" ] = self . assureString ( RR [ 0 ]                   )
      ########################################################################
      IMACT . append                  ( J                                    )
    ##########################################################################
    DB      . Close                   (                                      )
  ...
  def Prepare                    ( self                                    ) :
    ...
    LABELs = self . Translations [ "DomainsWidget" ] [ "Labels"              ]
  ...
  def CopyToClipboard        ( self                                        ) :
    ...
    return
  ############################################################################
  def DomainBelongings           ( self , DB , Uuid                        ) :
    ##########################################################################
    TLDTAB     = self . Tables   [ "TLD"                                     ]
    SLDTAB     = self . Tables   [ "SLD"                                     ]
    DOMTAB     = self . Tables   [ "Domains"                                 ]
