...
class UrlsWidget                   ( TreeDock                              ) :
  ...
  def PrepareItem                  ( self , JSON                           ) :
    ...
    ##########################################################################
    IT       . setText             ( 1 , str ( HNAME    )                    )
    IT       . setToolTip          ( 1 , UXID                                )
    IT       . setTextAlignment    ( 1 , Qt . AlignRight                     )
    ##########################################################################
    IT       . setText             ( 2 , str ( PROTOCOL )                    )
    IT       . setTextAlignment    ( 2 , Qt . AlignRight                     )
    ##########################################################################
    IT       . setText             ( 3 , str ( PORT     )                    )
  ...
  def loading                         ( self                               ) :
    ...
    for U in UUIDs                                                           :
      ...
      if ( ( RR not in [ False , None ] ) and ( len ( RR ) == 7 ) )          :
        ...
        J [ "Host"     ] = int        ( RR [ 4 ]                             )
        J [ "Protocol" ] = self . assureString ( RR [ 5 ]                    )
        J [ "Port"     ] = int        ( RR [ 6 ]                             )
        ######################################################################
        HUID            = J           [ "Host"                               ]
        if                            ( HUID > 0                           ) :
          ####################################################################
          QQ            = f"""select `name` from {HSTTAB}
                              where ( `uuid` = {HUID} ) ;"""
          QQ            = " " . join  ( QQ . split ( )                       )
          DB            . Query       ( QQ                                   )
          RR            = DB . FetchOne (                                    )
          ...
          if ( ( RR not in [ False , None ] ) and ( len ( RR ) == 1 ) )      :
            ##################################################################
            J [ "HName" ] = self . assureString ( RR [ 0 ]                   )
      ########################################################################
      IMACT . append                  ( J                                    )
    ##########################################################################
    DB      . Close                   (                                      )
  ...
  def FetchRegularDepotCount   ( self , DB                                 ) :
    ##########################################################################
    URLTAB = self . Tables     [ "URLs"                                      ]
    QQ     = f"select count(*) from {URLTAB} where ( `used` > 0 ) ;"
    DB     . Query             ( QQ                                          )
    ONE    = DB . FetchOne     (                                             )
    ##########################################################################
    if                         ( ONE == None                               ) :
  ...
  def Prepare                    ( self                                    ) :
    ...
    LABELs = self . Translations [ "UrlsWidget" ] [ "Labels"                 ]
  ...
  def CopyToClipboard        ( self                                        ) :
    ...
    return
  ############################################################################
  def UrlBelongings              ( self , DB , Uuid                        ) :
    ##########################################################################
